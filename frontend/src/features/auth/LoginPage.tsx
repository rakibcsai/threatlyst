    ...
    <main className="relative grid min-h-screen overflow-hidden bg-[#061019] text-slate-100 lg:grid-cols-[1.1fr_0.9fr]">
      ...
      <section className="relative hidden flex-col justify-between border-r border-slate-800/80 p-12 lg:flex xl:p-16">
        <div className="flex items-center gap-3">
          <span className="grid size-10 place-items-center rounded-xl border border-cyan-400/30 bg-cyan-400/10 text-cyan-300">
            <ShieldCheck className="size-6" />
          </span>
          <span className="text-lg font-bold tracking-wide">ThreatLyst</span>
        </div>
        <div className="max-w-xl">
          ...
          <h1 className="text-5xl font-semibold leading-[1.08] tracking-[-0.04em] text-white xl:text-6xl">
            Investigate threats.
            <br />
            <span className="text-gradient">Defend with clarity.</span>
          </h1>
          <p className="mt-6 max-w-lg text-base leading-7 text-slate-400">
            A focused command surface for AI-assisted security operations,
            threat analysis, and incident response.
          </p>
          <div className="mt-10 grid grid-cols-2 gap-3">
            <div className="surface-card p-4">
              <BrainCircuit className="mb-3 size-5 text-cyan-400" />
              <p className="text-sm font-semibold">AI-assisted analysis</p>
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Clear risk context for analyst decisions.
              </p>
            </div>
            <div className="surface-card p-4">
              ...
              <p className="mt-1 text-xs leading-5 text-slate-500">
                Admin, analyst, and viewer boundaries.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-[420px]">
          <div className="mb-10 lg:hidden">
            <div className="flex items-center gap-3">
              ...
              <span className="text-xl font-bold">ThreatLyst</span>
            </div>
          </div>
          ...
          <p className="mt-3 text-sm text-slate-500">
            Use your ThreatLyst username or email.
          </p>
          <form className="mt-8 space-y-5" onSubmit={submit}>
            ...
            <Button className="w-full" type="submit" disabled={isLoading}>
              {isLoading ? (
                ...
                <>
                  Enter ThreatLyst <ArrowRight className="size-4" />
                </>
              )}
            </Button>
          </form>
          <div className="mt-8 flex items-start gap-3 border-t border-slate-800 pt-6 text-xs leading-5 text-slate-600">
            <LockKeyhole className="mt-0.5 size-4 shrink-0" />
            <p>
              Your access token is held in memory only and is cleared when this
