...
import { Eye, Plus, RefreshCw, Siren } from 'lucide-react'
...
import { incidentStatuses, type IncidentResponse } from './incident-types'
      ...
      <div className="mx-auto max-w-[1600px]">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div>
            <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-cyan-400">
              <Siren className="size-3.5" /> Response operations
            </p>
            <h1 className="mt-2 text-2xl font-semibold tracking-[-0.025em] text-white md:text-3xl">
              Incidents
            </h1>
            <p className="mt-2 text-sm text-slate-500">
              Coordinate and track stored security incident records.
            </p>
          </div>
        ...
        {query.isLoading ? (
          ...
          <section className="surface-card mt-5 overflow-hidden">
            <div className="flex flex-col justify-between gap-2 border-b border-slate-800/80 px-4 py-3 sm:flex-row sm:items-center">
              <div>
                ...
                <p className="mt-1 text-[11px] text-slate-600">
                  Client-side view of all records returned by GET /api/incidents
                </p>
              </div>
              <p className="text-xs text-slate-500">
                ...
                of {query.data.length} incidents
              </p>
            </div>
            {query.data.length > 0 && (
              <QueueToolbar
            ...
            {filtered.length > 0 ? (
              <IncidentsTable incidents={filtered} onSelect={setSelected} />
            ) : (
              <QueueEmpty filtered={query.data.length > 0} noun="incidents" />
            )}
          </section>
        )}
