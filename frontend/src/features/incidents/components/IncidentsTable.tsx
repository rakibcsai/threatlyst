import { Eye, Siren, UserRound } from 'lucide-react'
...
import type { IncidentResponse } from '../incident-types'
          ...
          <tbody>
            {incidents.map((incident) => (
              <tr
                ...
                onKeyDown={(event) =>
                  activateOperation(event, () => onSelect(incident))
                }
              ...
              >
                ...
                <td className="px-4 py-4 font-mono text-xs text-slate-500">
                  User {incident.created_by_user_id}
                </td>
                <td className="px-4 py-4 text-xs text-slate-500">
                  {incident.assigned_to_user_id === null
                    ? 'Unassigned'
                    : `User ${incident.assigned_to_user_id}`}
                </td>
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="flex items-center gap-2 text-sm font-semibold text-slate-200">
                  <Siren className="size-4 text-rose-400" /> {incident.title}
                </p>
                ...
                <time
                  ...
                  title={incident.created_at}
                >
                  Created {formatOperationTimestamp(incident.created_at)}
                </time>
              </div>
              <SeverityBadge severity={incident.severity} />
            </div>
